...
def art2skirt(ds, region, center, savenamebase):
    ...
    outs = np.column_stack([xs, ys, zs, es, ms, Zs, ts])
    ...
    headerg = (
        ...
        "column 5: ymax (pc)\n"
        "column 6: zmax (pc)\n"
        "Column 7: gas mass (Msun)\n"
        "Column 8: metallicity (1)\n"
        "Column 9: temperature (K)\n"
        )
    outg = np.column_stack([x0g, y0g, z0g, x1g, y1g, z1g, mg, Zg, Tg])
...
if __name__ == "__main__":
    ...
    art2skirt(ds, region, center, "analysis/skirt/skirt_a%.4f"%scale_a)